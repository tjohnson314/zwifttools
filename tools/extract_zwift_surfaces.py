...
def _markers(block: str) -> list[tuple[float, float, int]]:
    ...
    A marker with only ONE road-time defaults the missing endpoint to the road
    boundary (missing ``m_roadTime1`` -> 0, missing ``m_roadTime2`` -> 1); only a
    marker with NEITHER road-time is position-less and skipped. Verified in-game
    on Makuri road 129: marker 1290001 (style-less, visible, only
    ``m_roadTime2=0.2918``) renders asphalt from the road start to 0.2918.
    """
